...
```{r echo=TRUE, eval=FALSE, fig.width=14, fig.height=7}
xyplot(BPchange ~ log(Dose) | Animal * Treatment, Rabbit, xlab = "log(Dose) of Phenylbiguanide", ylab = "Change in blood pressure (mm Hg)", subscripts = T, aspect = "fill", panel = function(x, y, subscripts) {
  panel.grid()
  panel.xyplot(x, y)
...
```{r echo=FALSE, eval=TRUE, fig.width=14, fig.height=7}
xyplot(BPchange ~ log(Dose) | Animal * Treatment, Rabbit, xlab = "log(Dose) of Phenylbiguanide", ylab = "Change in blood pressure (mm Hg)", subscripts = T, aspect = "fill", panel = function(x, y, subscripts) {
  panel.grid()
  panel.xyplot(x, y)
  sp <- spline(x, fitted(R.nlme2)[subscripts])
...
```{r echo=FALSE, eval=TRUE, fig.width=14, fig.height=7, message=FALSE, warning=FALSE}
library(splines)
Rabbit2 <- cbind(Rabbit, fitted=fitted(R.nlme2))
p <- ggplot(data=Rabbit2, aes(x=log(Dose), y=BPchange))
p <- p + geom_point(colour="red") + facet_grid(Treatment ~ Animal)
